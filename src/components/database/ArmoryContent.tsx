...
export function ArmoryContent({ activeTab }: { activeTab: ArmoryTab }) {
    ...
    // Measure exact card width from a hidden grid cell (same gridClass)
    ...
    const saveWeapon = () => {
        ...
        if (editingWeapon) {
            ...
            const newWeapon: Weapon = {
                ...
                skillBonus: weaponForm.skillBonus,
                grantsArmor: weaponForm.grantsArmor,
    ...
    if (activeTab === 'Gear') {
        return (
            <>
                        ...
                        <div className="space-y-3 py-2">
                            ...
                            <div className="flex gap-4 items-center flex-wrap">
                                ...
                                <select
                                    ...
                                    onChange={(e) => setWeaponForm({ ...weaponForm, skillReq: (e.target.value as Weapon['skillReq']) || undefined, skillBonus: undefined })}
                                ...
                                {weaponForm.skillReq && (
                                    <div className="flex items-center gap-1.5">
                                        <Label className="font-mono-tech text-xs uppercase tracking-widest text-muted-foreground">Bonus</Label>
                                        <Input type="number" value={weaponForm.skillBonus ?? ''} onChange={(e) => setWeaponForm({ ...weaponForm, skillBonus: e.target.value === '' ? undefined : Number(e.target.value) })} placeholder="—" className="bg-black border-border font-mono-tech text-xs h-6 w-14 px-1" />
                                    </div>
                                )}
                            </div>
                            <div className="flex gap-4 items-center">
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" checked={(weaponForm.grantsArmor ?? 0) > 0} onChange={(e) => setWeaponForm({ ...weaponForm, grantsArmor: e.target.checked ? (weaponForm.grantsArmor || 1) : 0 })} className="accent-white" />
                                    <span className="font-mono-tech text-xs uppercase text-white">Armor</span>
                                </label>
                                {(weaponForm.grantsArmor ?? 0) > 0 && (
                                    <Input type="number" min={1} value={weaponForm.grantsArmor ?? 1} onChange={(e) => setWeaponForm({ ...weaponForm, grantsArmor: Number(e.target.value) })} className="bg-black border-border font-mono-tech text-xs h-6 w-14 px-1" />
                                )}
                            </div>
                            <div className="space-y-2">
                ...
                {/* Hidden measurement grid — same gridClass, measures exact card cell width */}
                {gearViewMode === 'card' && (
                ...
                {/* Weapons grid — CARD view (flex-wrap for stacking) */}
                {gearViewMode === 'card' && (() => {
                    const STRIP_W = 24; // w-6 = 1.5rem = 24px
                    ...
                    const cw = measuredCardW; // measured card width (0 before first paint)

                    // Build flat list of flex items
                    ...
                    for (const [weaponId, group] of groups) {
                        ...
                        if (!isMulti) {
                            items.push(
                                <div key={weaponId} style={{ ...cardStyle, width: cw || undefined }}>
                            ...
                            for (const { variant } of group.variants) {
                                items.push(
                                    <div key={weaponId + '-' + variant.factionId} style={{ ...cardStyle, width: cw || undefined }} className="cursor-pointer" onClick={() => toggleExpanded(weaponId)}>
                            ...
                            // Collapsed stack: strips + front card, card keeps exact measured width
                            const [front, ...behind] = group.variants;
                            const totalW = cw > 0 ? cw + behind.length * STRIP_W : undefined;
                            items.push(
                                <div key={weaponId + '-stack'} style={{ ...cardStyle, width: totalW }} className="cursor-pointer" onClick={() => toggleExpanded(weaponId)}>
                                    <div className="flex items-stretch">
                                        ...
                                        <div style={{ width: cw || undefined }} className="shrink-0">
                    ...
                    return <div className="flex flex-wrap gap-4">{items}</div>;
                })()}
